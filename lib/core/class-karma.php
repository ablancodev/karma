...
class Karma {
	...
	public static function set_karma ( $karma, $user_id, $info = array() ) {
		...
		if ( isset( $info['datetime'] ) ) {
			...
			$info['datetime'] = $now = date('Y-m-d H:i:s', time() );
		}
	...
	public static function get_karmas ( $limit = null, $order_by = null, $order = null, $output = OBJECT ) {
		...
		$result = $wpdb->get_results("SELECT * FROM " . Karma_Database::karma_get_table( "users" ) . $order_by_str . $order_str . $limit_str, $output );
	...
	public static function update_karma( $karma, $user_id, $info = array() ) {
		global $wpdb;
		...
		$rows_affected = $wpdb->update( Karma_Database::karma_get_table("users"), $values , array( 'user_id' => $user_id ) );
